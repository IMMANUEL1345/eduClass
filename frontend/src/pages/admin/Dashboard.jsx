...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const ov = await analyticsAPI.overview().catch(() => null);
        if (ov) setOverview(ov.data.data);
      } catch {}

      try {
        const tr = await analyticsAPI.attendanceTrend({}).catch(() => null);
        if (tr) setTrend(tr.data.data.slice(-7));
      } catch {}

      try {
        const ann = await commsAPI.announcements().catch(() => null);
        if (ann) setAnnouncements(ann.data.data.slice(0, 5));
      } catch {}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <StatCard label="Total students"    value={overview?.students ?? 0}              sub="Active enrolments"     color="blue"   />
            <StatCard label="Teachers"          value={overview?.teachers ?? 0}              sub="Active staff"          color="green"  />
            <StatCard label="Classes"           value={overview?.classes  ?? 0}              sub="This academic year"    color="purple" />
